...
def test_ddp_choice_sharded(tmpdir, ray_start_2_cpus, seed):
    ...
    class CB(Callback):
        def on_fit_start(self, trainer, pl_module):
            assert isinstance(trainer.accelerator.strategy, RayShardedStrategy)
            raise SystemExit()

    model = BoringModel()
    trainer = Trainer(
        fast_dev_run=True,
        plugins=[RayShardedStrategy(num_workers=2)],
        callbacks=[CB()],
    )
...
def test_ddp_sharded_plugin_resume_from_checkpoint_downsize(
    ...
    model = BoringModel()
    trainer = Trainer(
        plugins=[RayShardedStrategy(num_workers=2)], fast_dev_run=True)
    ...
    trainer = Trainer(
        plugins=[RayShardedStrategy(num_workers=1)],
        fast_dev_run=True,
        resume_from_checkpoint=checkpoint_path)
