...
def test_train(tmpdir, ray_start_2_gpus, num_workers):
    ...
    model = BoringModel()
    plugin = RayStrategy(num_workers=num_workers, use_gpu=True)
...
def test_model_to_gpu(tmpdir, ray_start_2_gpus):
    ...
    plugin = RayStrategy(num_workers=2, use_gpu=True)
    trainer = get_trainer(
...
def test_multi_node(tmpdir):
    ...
    model = BoringModel()
    plugin = RayStrategy(num_workers=num_gpus, use_gpu=True)
    trainer = get_trainer(tmpdir, plugins=[plugin])
    train_test(trainer, model)
