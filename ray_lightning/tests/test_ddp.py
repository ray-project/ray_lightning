...
def test_actor_creation(tmpdir, ray_start_2_cpus, num_workers):
    ...
    strategygy = RayStrategy(num_workers=num_workers)
...
def test_global_local_ranks(ray_start_4_cpus):
    ...
    strategygy = RayStrategy(num_workers=4, use_gpu=False)
    strategygy._configure_launcher()

    # 2 workers on "Node 1", 2 workers on "Node 2"
    strategygy._launcher._workers = [
        Node1Actor.remote(),
        Node1Actor.remote(),
        Node2Actor.remote(),
        Node2Actor.remote()
    ...
    global_to_local = strategygy._launcher.get_local_ranks()
...
def test_resource_override(ray_start_2_cpus):
    ...
    strategygy = RayStrategy(num_workers=1, num_cpus_per_worker=2, use_gpu=True)
    assert strategygy.num_cpus_per_worker == 2
    assert strategygy.use_gpu
    ...
    assert strategygy.num_cpus_per_worker == 3
    assert strategygy.use_gpu
    ...
    assert strategygy.num_cpus_per_worker == 2
    assert strategygy.num_gpus_per_worker == 2
    assert strategygy.use_gpu
...
def test_distributed_sampler(tmpdir, ray_start_2_cpus):
    ...
    strategygy = RayStrategy(num_workers=2)
    trainer = get_trainer(
        tmpdir, strategy=[strategygy], callbacks=[DistributedSamplerCallback()])
    trainer.fit(model)
...
def test_test_with_dataloader_workers(tmpdir, ray_start_2_cpus, seed):
    ...
    model = BoringModel()
    strategygy = RayStrategy(num_workers=1, use_gpu=False)
    trainer = get_trainer(
...
def test_load(tmpdir, ray_start_2_cpus, num_workers):
    ...
    model = BoringModel()
    strategygy = RayStrategy(num_workers=num_workers, use_gpu=False)
    trainer = get_trainer(tmpdir, strategy=[strategygy])
...
def test_unused_parameters(tmpdir, ray_start_2_cpus):
    ...
    trainer = get_trainer(
        tmpdir, strategy=[strategygy], callbacks=[UnusedParameterCallback()])
    trainer.fit(model)
...
def test_metrics(tmpdir, ray_start_2_cpus):
    ...
    model = XORModel()
    strategygy = RayStrategy(num_workers=2, find_unused_parameters=False)
    trainer = get_trainer(
        tmpdir,
        strategy=[strategygy],
        max_epochs=1,
